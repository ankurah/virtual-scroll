...
export async function waitForWasm(page: Page): Promise<void> {
  ...
  await page.waitForFunction(() => window.wasm !== null, { timeout: 30000 });

  // Wait for testHelpers to be set (component mounted)
  await page.waitForFunction(() => window.testHelpers !== null, { timeout: 30000 });

  // Give a bit more time for everything to stabilize
  await page.waitForTimeout(500);

  // Verify testHelpers is actually working
  const helpersWork = await page.evaluate(() => {
    return window.testHelpers !== null && typeof window.testHelpers.getItemCount === 'function';
  });

  if (!helpersWork) {
    throw new Error('testHelpers not properly initialized');
  }
}
